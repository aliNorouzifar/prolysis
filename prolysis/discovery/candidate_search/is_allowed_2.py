...
def is_allowed_multi(SS1,SS2,rules_lookup,to_be_checked, lowest_cost):
    # dim = 'support'
    # dim = rules_lookup[2]
    dim_cont = rules_lookup[2][0]
    dim_des = rules_lookup[2][1]

    rules_cont = rules_lookup[0][0]
    rules_des = rules_lookup[0][1]
    ...
    exclude = set()
    penalty_cont = {'seq': 0, 'exc': 0, 'par': 0, 'loop': 0, 'loop_tau': 0, 'exc_tau': 0}
    for r in rules_cont:
        x =r["template"]
        if len(r['parameters'])==2:
            if (r['parameters'][0][0] not in SS) or (r['parameters'][1][0] not in SS):
        ...
        #     print('wait')
        for ex in exclude_new:
            penalty_cont[ex] += r[dim_cont]
        # if r[dim_cont]>=abs_thr:
        exclude = exclude | exclude_new

        if all([penalty_cont[ct]>lowest_cost for ct in to_be_checked]):
            # print(f'visited')
            break
    # print(to_be_checked)
    lowest_cost = min(lowest_cost,min([penalty_cont[ct] for ct in to_be_checked]))
    # print(f'{lowest_cost}')
    ...
    for r in rules_des:
        ...
        #     print('wait')
        for ex in exclude_new:
            penalty_des[ex] += r[dim_des]
            rules_dev_des[ex].append(r)


    return exclude, set(), penalty_cont, penalty_des ,lowest_cost, rules_dev_des



def is_allowed_single(activity,rules_lookup):
    rules =  rules_lookup[0][0]
    lookup_table = rules_lookup[1]
    # dim = 'support'
    dim = rules_lookup[2][0]
    exclude = set()
